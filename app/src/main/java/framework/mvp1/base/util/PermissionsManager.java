    ...
    public void checkPermissions(Activity context
        ...
        for (int i = 0; i < permissions.length; i++) {
            if (ContextCompat.checkSelfPermission(context, permissions[i])
                ...
                boolean isRequested = false;//遵循上架规范，已经申请过的权限不再申请
                try {
                    isRequested = JnCache.getCache(context, "cp_" + permissions[i]);
                } catch (Exception e) {
                }
                if (!isRequested) {
        ...
        if (mPermissionList.size() > 0) {//有权限没有通过，需要申请
            String[] requestPerssion = new String[mPermissionList.size()];
            for (int j = 0; j < mPermissionList.size(); j++) {
                try {
//                    HawkUtils.getInstance().saveCacheData("cp_" + mPermissionList.get(j), true);
                    JnCache.saveCache("cp_" + mPermissionList.get(j), true);
                } catch (Exception e) {
                }
                requestPerssion[j] = mPermissionList.get(j);
            }
            ActivityCompat.requestPermissions(context, requestPerssion, mRequestCode);
        } else {
    ...
    private void showSystemPermissionsSettingDialog(final Activity context) {
        ...
        mPermissionDialog.getButton(AlertDialog.BUTTON_POSITIVE).setTextColor(Color.BLUE);
        mPermissionDialog.getButton(DialogInterface.BUTTON_NEGATIVE).setTextColor(Color.BLACK);
    }
